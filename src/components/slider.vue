<template>
  <div id="dragContainer" ref="dragContainer">
    <!--拖动时的绿色区域-->
    <div id="dragBg" ref="dragBg"></div>
    <div id="dragText" ref="dragText">{{text}}</div>
    <!--拖动滑块-->
    <div id="dragHandler" ref="dragHandler" class="dragHandlerBg" @mousedown="onDragHandlerMouseDown"></div>
  </div>
</template>

<style lang="less">
  /* 滑动控件容器,亮灰色背景 */
  #dragContainer {
    position: relative;
    display: inline-block;
    background: #e8e8e8;
    width: 300px;
    height: 33px;
    border: 2px solid #e8e8e8;
  }

  /* 滑块左边部分,绿色背景 */
  #dragBg {
    position: absolute;
    background-color: #7ac23c;
    width: 0px;
    height: 100%;
    z-index: 50;
  }

  /* 滑动验证容器文本 */
  #dragText {
    position: absolute;
    width: 100%;
    height: 100%;
    /* 文字水平居中 */
    text-align: center;
    /* 文字垂直居中,这里不能用百分比,因为百分比是相对原始line-height的,而非div高度 */
    line-height: 33px;
    /* 文本不允许选中 */
    user-select: none;
    -webkit-user-select: none;
  }

  /* 滑块 */
  #dragHandler {
    position: absolute;
    width: 40px;
    height: 100%;
    cursor: move;
  }

  /* 滑块初始背景 */
  .dragHandlerBg {
    background: #fff no-repeat center url("data:image/png;base64,");
  }

  /* 验证成功时的滑块背景 */
  .dragHandlerOkBg {
    background: #fff no-repeat center url("data:image/png;base64,");
  }
</style>

<script>
  export default {
    data() {
      return {
        msg: 'hello vue',
        text: '拖动滑块验证',
        isValidate: false,
        maxHandlerOffset: ''
      }
    },
    methods: {
      onDragHandlerMouseDown() {
        this.$refs.dragHandler.setAttribute('class', 'dragHandlerBg');
        document.addEventListener('mousemove', this.onDragHandlerMouseMove);
        document.addEventListener('mouseup', this.onDragHandlerMouseUp);
      },
      onDragHandlerMouseMove() {
        // 最大滑动距离(滑动条宽度减去滑动块宽度)
        this.maxHandlerOffset = Number(this.$refs.dragContainer.clientWidth - this.$refs.dragHandler.clientWidth);
        let left = event.clientX - this.$refs.dragHandler.clientWidth / 2;
        if (left < 0) {
          left = 0;
        } else if (left > this.maxHandlerOffset) {
          left = this.maxHandlerOffset;
          this.verifySucc();
        }
        this.$refs.dragHandler.style.left = left + "px"; // 滑块位置
        this.$refs.dragBg.style.width = this.$refs.dragHandler.style.left; // 左侧绿色区域宽度
      },
      onDragHandlerMouseUp() {
        document.removeEventListener('mousemove', this.onDragHandlerMouseMove);
        document.removeEventListener('mouseup', this.onDragHandlerMouseUp);
        this.$refs.dragHandler.style.left = 0;
        this.$refs.dragBg.style.width = 0;
      },
      //验证成功
      verifySucc() {
        this.isValidate = true;
        this.text = '验证通过';
        this.$refs.dragBg.style.zIndex = 0;
        this.$refs.dragContainer.style.color = 'white';
        this.$refs.dragHandler.setAttribute('class', 'dragHandlerOkBg');
        this.$refs.dragHandler.removeEventListener('mousedown', this.onDragHandlerMouseDown);
        document.removeEventListener('mousemove', this.onDragHandlerMouseMove);
        document.removeEventListener('mouseup', this.onDragHandlerMouseUp);
      }
    }
  }
</script>
